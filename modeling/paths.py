...
# Sibling GrIS 2024 training table name (present only on the researcher machine)
PAIRS_CSV_GRIS_2024 = os.path.join(PAIRS_DIR, "is2_s2_pairs_gris_2024_train7744.csv")

# Trained RF artifacts (gitignored under out/; exist on the researcher machine)
RF_MODEL_DIR = os.path.join(MODELS_DIR, "random_forest")
RF_MODEL_JOBLIB = os.path.join(RF_MODEL_DIR, "random_forest.joblib")
RF_METRICS_JSON = os.path.join(RF_MODEL_DIR, "metrics.json")
RF_CV_PREDICTIONS_CSV = os.path.join(RF_MODEL_DIR, "diagnostics", "cv_predictions.csv")
RF_INFERENCE_DIR = os.path.join(RF_MODEL_DIR, "inference")
RF_INFERENCE_CSV = os.path.join(RF_INFERENCE_DIR, "gris_lake_depth_pred.csv")
